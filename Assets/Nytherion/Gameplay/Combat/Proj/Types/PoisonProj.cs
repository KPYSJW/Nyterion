...
namespace Nytherion.GamePlay.Combat
{
    ...
    public class PoisonProj : MonoBehaviour, IProj, IProjModifier
    {
        ...
        // IProj 구현: RangedWeapon이 발사 각도를 지정하고 속도를 인가할 때 호출
        ...
        // IProjModifier 구현: CollisionObject가 충돌 시 OnHit를 호출
        public bool OnHit(Collider2D target)
        {
            if (target.CompareTag("Enemy"))
            {
                // 적 충돌 시 독 상태 이상 부여
                if (target.TryGetComponent<StatusEffectManager>(out StatusEffectManager effectManager))
                {
