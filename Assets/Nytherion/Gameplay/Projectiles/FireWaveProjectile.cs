...
namespace Nytherion.GamePlay.Combat
{
    ...
    public class FireWaveProjectile : MonoBehaviour, IProjectile, IProjectileEffect
    {
        ...
        // IProjectile 구현: RangedWeapon 등에서 속도를 인가할 때 호출
        ...
        // IProjectileEffect 구현: CollisionObject 충돌 시 호출
        public bool OnHit(Collider2D target)
        {
            if (target.CompareTag("Enemy"))
            {
                // 기본적으로 관통 여부 필드 리턴 (기본값 false이므로 충돌 즉시 소멸)
                // 유물에 의해 추가된 PiercingEffect가 있을 시, 그 컴포넌트가 true를 리턴하므로 이 값과 무관하게 관통 가능
